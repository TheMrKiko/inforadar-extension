...
chrome.action.setTitle({ title: "Clique para ver a análise desta página no InfoRadar." })

chrome.action.onClicked.addListener(async (ctab) => {
    ...
    let tab = await chrome.tabs.create({
        url: `https://inforadar.inesc-id.pt?url=${encodeURIComponent(ctab.url)}`
    });
...
chrome.tabs.onActivated.addListener((activeInfo) => {
    chrome.tabs.get(activeInfo.tabId, (tab) => {
        updateBasedOnUrl(tab.url, tab.id);
    });
});

chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
    if (changeInfo.url != undefined)
        updateBasedOnUrl(changeInfo.url, tabId);
});

function updateBasedOnUrl(url, tabId) {
    try {
        const urlObj = new URL(url);

        if (urlObj.hostname.includes("inforadar.inesc-id.pt")) {
            //chrome.action.setPopup({ tabId: tabId, popup: '' });
            chrome.action.setIcon({ path: 'icon32-disabled.png', tabId: tabId });
            console.log('not matching');
        } else {
            //chrome.action.setPopup({ tabId: tabId, popup: '../html/popup.html' });
            chrome.action.setIcon({ path: 'icon32.png', tabId: tabId });
            console.log('matched');
        }
